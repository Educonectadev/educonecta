...
CREATE TABLE "Schedule" (id SERIAL PRIMARY KEY, dayOfWeek INT NOT NULL CHECK (dayOfWeek BETWEEN 1 AND 7), startTime TIME NOT NULL, endTime TIME NOT NULL, classroom VARCHAR(255), shift VARCHAR(50) DEFAULT 'MAÑANA', courseId INT NOT NULL REFERENCES "Course"(id) ON DELETE CASCADE, institutionId INT NOT NULL REFERENCES "Institution"(id) ON DELETE CASCADE, createdAt TIMESTAMPTZ DEFAULT NOW(), updatedAt TIMESTAMPTZ DEFAULT NOW());
