  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-40 bg-white/90 backdrop-blur-md border-b border-gray-100">
        <div className="flex items-center justify-between px-5 md:px-8 py-3 md:py-4 max-w-6xl mx-auto w-full text-gray-900">
          <Link href="/" className="text-sm md:text-base font-semibold tracking-tight text-gray-900 hover:opacity-60 transition-opacity duration-200">
            EduConecta
          </Link>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-44 bg-white border border-gray-100 rounded-xl shadow-lg py-1.5 z-50 animate-fade-in">
                    ...
                    {(session.user.role === "PARENT" || session.user.role === "TEACHER" || session.user.role === "INSTITUTIONAL_ADMIN") && (
                      <button
                        ...
                        className="w-full text-left px-4 py-2 text-sm text-gray-500 hover:text-gray-900 hover:bg-gray-50 transition-all duration-200"
                      >
                        Perfil
                      </button>
                    )}
