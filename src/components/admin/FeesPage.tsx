    ...
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      {showForm && (
        <div className="rounded-2xl bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 p-6 space-y-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{editing ? "Editar" : "Nueva"} cuota</h3>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowForm(false)} className="rounded-xl px-5 py-2 text-sm text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-white">
              Cancelar
            </button>
          </div>
        </div>
      ...
      <div className="space-y-2">
          ...
          >
            <div className="flex items-center gap-4">
              <div className="flex flex-col">
                <span className="text-sm font-medium text-gray-900 dark:text-white">{fee.name}</span>
                <span className="text-xs text-gray-400 dark:text-zinc-500">
                  {typeLabels[fee.type]} · S/ {fee.amount.toFixed(2)} · Vence día {fee.dueDay}
                </span>
              </div>
              <span className="text-xs text-gray-500 dark:text-zinc-600">{fee.academicYear}</span>
            </div>
            <div className="flex gap-2">
              <button onClick={() => openEdit(fee)} className="text-gray-400 dark:text-zinc-500 hover:text-emerald-500 dark:hover:text-emerald-400">
                <Pencil className="w-4 h-4" />
              </button>
              <button onClick={() => remove(fee.id)} className="text-gray-400 dark:text-zinc-500 hover:text-red-500 dark:hover:text-red-400">
        ...
        {fees.length === 0 && (
          <p className="text-sm text-gray-400 dark:text-zinc-500 text-center py-8">No hay cuotas registradas.</p>
        )}
      </div>
    </div>
  )
