...
export default async function DisciplinaPage() {
  ...
  return (
    <div>
      ...
      {records.length === 0 ? (
        <p className="text-sm text-gray-500">No hay registros disciplinarios.</p>
      ) : (
        <div className="grid gap-3">
          {records.map((r: { id: number; type: string; description: string; date: Date; isResolved: boolean; student: { firstName: string; lastName: string } }) => (
            <div key={r.id} className="bg-gray-50 border border-gray-200 rounded-[25px] p-6">
              ...
              <div className="flex items-center gap-3 mt-3 text-xs text-gray-500">
                ...
                {r.isResolved && <span className="text-emerald-600 font-medium">Resuelto</span>}
              </div>
            </div>
          ))}
        </div>
