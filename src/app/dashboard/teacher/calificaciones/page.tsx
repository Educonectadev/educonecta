  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-8">
        <h1 className="text-2xl font-bold tracking-tight dark:text-white">Calificaciones</h1>
        <Link
      ...
      {grades.length === 0 ? (
        <p className="text-sm text-gray-500 dark:text-zinc-500">No hay calificaciones registradas.</p>
      ) : (
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {grades.map((g: { id: number; student: { firstName: string; lastName: string }; course: { name: string }; evaluationName: string; grade: number; evaluationDate: Date | null }) => (
                <tr key={g.id} className="flex flex-col md:table-row border border-gray-100 dark:border-zinc-800/50 md:border-0 rounded-[30px] p-4 md:p-0 mb-3 md:mb-0 hover:bg-gray-50/50 dark:hover:bg-zinc-900/50 transition-colors">
                  ...
                  <td className="flex justify-between md:table-cell px-0 md:px-4 py-1 md:py-3 text-gray-600 dark:text-zinc-400">
                    <span className="md:hidden text-xs uppercase tracking-widest text-gray-500 dark:text-zinc-500">Evaluación</span>
                    <span>{g.evaluationName}</span>
                  </td>
                  <td className="flex justify-between md:table-cell px-0 md:px-4 py-1 md:py-3 font-medium dark:text-white">
