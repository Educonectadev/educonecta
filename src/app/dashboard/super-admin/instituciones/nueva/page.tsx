              ...
              {form.shifts.length > 0 && (
                <div className="mt-3 space-y-2">
                  {form.shifts.map((id) => {
                    ...
                    return (
                      <div key={id} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 bg-white rounded-[20px] border border-gray-200 px-4 py-3 sm:py-2.5">
                        <span className="text-xs font-medium text-gray-600 sm:w-14 shrink-0">{opt?.label ?? id}</span>
                        <div className="flex items-center gap-2 flex-1">
                          <input
                            type="time"
                            value={time.start}
                            onChange={(e) => setForm((p) => ({ ...p, shiftTimes: { ...p.shiftTimes, [id]: { ...(p.shiftTimes[id] ?? { start: "08:00", end: "17:00" }), start: e.target.value } } }))}
                            className="flex-1 min-w-0 rounded-[15px] border border-gray-200 px-3 py-1.5 text-xs text-black focus:border-black focus:outline-none focus:ring-1 focus:ring-black bg-white"
                          />
                          <span className="text-xs text-gray-400 shrink-0">a</span>
                          <input
                            type="time"
                            value={time.end}
                            onChange={(e) => setForm((p) => ({ ...p, shiftTimes: { ...p.shiftTimes, [id]: { ...(p.shiftTimes[id] ?? { start: "08:00", end: "17:00" }), end: e.target.value } } }))}
                            className="flex-1 min-w-0 rounded-[15px] border border-gray-200 px-3 py-1.5 text-xs text-black focus:border-black focus:outline-none focus:ring-1 focus:ring-black bg-white"
                          />
                        </div>
                      </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Breve descripción o misión de la institución"
                />
              </div>
            </div>
          </div>
